...
def add_active_escrow(
    ...
    conn.commit()
    conn.close()
    logger.info(f"✅ Active escrow added: Room {room_num} | Seller: {seller_username} | Buyer: {buyer_username}")
...
async def show_escrows_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("SELECT room_id, room_num, seller_username, buyer_username, start_time FROM active_escrows")
    results = c.fetchall()
    conn.close()
...
async def escrow_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await msg.reply_text(
        ...
        f"📝 **Rules:**\n"
        f"• ONLY ADMIN {ADMIN_USERNAME} can complete or cancel\n"
        f"• Seller & Buyer DON'T SEND ANY ASSETS WITHOUT ADMIN CONFIFORMATION\n"
        f"• Admin will resolve disputes\n\n"
    )
