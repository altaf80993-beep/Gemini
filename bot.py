...
def init_db():
    ...
    c = conn.cursor()
    # Users table - verified = 0 by default
...
async def handle_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Auto-save on join with verified=0, send welcome message in group only"""
    ...
    # Save to database with verified = 0
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("INSERT OR IGNORE INTO users (user_id, username, first_name, join_time, verified) VALUES (?, ?, ?, ?, 0)",
              (user.id, (user.username or user.first_name).lower(), user.first_name, time.time()))
    conn.commit()
    conn.close()
    
    # Welcome message in group only - as requested
    welcome_text = f"welcome {username} to escrow spartans group please click /start and verify your id to make deals in this group"
...
async def filter_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.from_user.username and msg.from_user.username.lower() == ADMIN_USERNAME.replace('@', '').lower():
        return
...
async def username_change_check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Username change alert - only when actually changed"""
    msg = update.message
    ...
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("SELECT username FROM users WHERE user_id=?", (user.id,))
    result = c.fetchone()
    
    if not result:
        # New user, just save
        c.execute("INSERT OR IGNORE INTO users (user_id, username, first_name, join_time, verified) VALUES (?, ?, ?, ?, 0)",
                  (user.id, new_username, user.first_name, time.time()))
        conn.commit()
        conn.close()
        return
    ...
    # Only send alert if username actually changed
    if old_username and old_username != new_username and user.username:
        main_group_id = get_main_group_id()
        if main_group_id:
        ...
        # Update database
        update_username_in_db(user.id, new_username)
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Verify user when they click /start - works in group or private"""
    user = update.effective_user
    username = f"@{user.username}" if user.username else user.first_name
    
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("UPDATE users SET verified=1 WHERE user_id=?", (user.id,))
    # Agar user exist nahi karta to insert karo
    if c.rowcount == 0:
        c.execute("INSERT INTO users (user_id, username, first_name, join_time, verified) VALUES (?, ?, ?, ?, 1)",
                  (user.id, (user.username or user.first_name).lower(), user.first_name, time.time()))
    conn.commit()
    conn.close()
    
    await update.message.reply_text(
        f"✅ **Verification Successful!**\n\n{username}, aap verified ho chuke hain.\n\n"
        f"Ab aap group mein `/escrow @username` command use kar sakte hain.\n\n"
        f"⚠️ For problems: Contact {ADMIN_USERNAME}",
        parse_mode='Markdown'
    )

async def escrow_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Create escrow room - only if both users are verified"""
    msg = update.message
    ...
    if not is_user_verified(u1_user.id):
        await msg.reply_text("❌ Aap verified nahi ho! Pehle /start karein.")
        return
    
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("SELECT user_id, verified FROM users WHERE username=?", (u2_username,))
    u2_data = c.fetchone()
    conn.close()

    if not u2_data:
        await msg.reply_text(f"❌ User @{u2_username} not found! Ask them to /start first.")
        return
    
    if u2_data[1] == 0:
        await msg.reply_text(f"❌ User @{u2_username} verified nahi hai! Unhe pehle /start karna hoga.")
        return
...
async def handle_security(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if new_user.username and new_user.username.lower() == ADMIN_USERNAME.replace('@', '').lower():
        return
...
async def complete_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = update.effective_user
    if not user.username or user.username.lower() != ADMIN_USERNAME.replace('@', '').lower():
