...
ADMIN_ID = int(os.getenv("ADMIN_ID", "6722137021"))  # Your admin ID
ADMIN_USERNAME = "@crypto_8099"  # Your username
...
async def escrow_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(
            ...
            text=(
                ...
                f"⚠️ **PERMISSIONS:**\n"
                f"• ONLY ADMIN (@{ADMIN_USERNAME}) can use /complete\n"
    ...
    await msg.reply_text(
        ...
        f"📝 **Important Rules:**\n"
        f"• ONLY ADMIN ({ADMIN_USERNAME}) can complete the deal (/complete)\n"
    ...
    try:
        await context.bot.send_message(
            ...
            text=(
                ...
                f"• ONLY ADMIN ({ADMIN_USERNAME}) can COMPLETE the deal\n\n"
...
async def complete_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ONLY ADMIN (@crypto_8099) can complete
    if user_id != ADMIN_ID:
        await msg.reply_text(
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not (is_seller or is_buyer or is_admin):
        await msg.reply_text(
            f"❌ Only Seller (@{escrow['seller_username']}), Buyer (@{escrow['buyer_username']}), or Admin {ADMIN_USERNAME} can cancel this deal!"
        )
        return
    ...
    if is_seller:
        ...
        cancelled_by_display = f"{ADMIN_USERNAME} (Admin)"
