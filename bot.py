...
async def complete_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # CRITICAL: ONLY ADMIN can complete
    if user_id != ADMIN_ID:
        await msg.reply_text(
    ...
    # Notify in main group that deal is completed
    try:
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check if user is authorized to cancel (Seller, Buyer, or Admin)
    is_authorized = (
    ...
    # Notify in main group that deal is cancelled
    try:
        main_group_id = int(GROUP_ID) if str(GROUP_ID).lstrip('-').isdigit() else GROUP_ID
        await context.bot.send_message(
            chat_id=main_group_id,
    ...
    logger.info(f"Escrow cancelled by {cancelled_by}: {seller_username} -> {buyer_username}")
