...
def save_user(user):
    """Save user to database - exact username, no lowercase conversion for storage"""
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    username = user.username if user.username else user.first_name
    c.execute("INSERT OR REPLACE INTO users (user_id, username, first_name, join_time) VALUES (?, ?, ?, ?)",
              (user.id, username, user.first_name, time.time()))
    conn.commit()
    conn.close()
    logger.info(f"User saved/updated: ID={user.id}, Username={username}")

def get_user_by_username(username):
    """Get user from database by username (case-insensitive search)"""
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    # Remove @ if present
    clean_username = username.replace('@', '').lower()
    c.execute("SELECT user_id, username FROM users WHERE LOWER(username)=?", (clean_username,))
    result = c.fetchone()
    conn.close()
    return result

def is_admin(user):
    """Check if user is admin by username (case-insensitive)"""
...
async def handle_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Save user on join, send welcome message - SIRF EK BAAR"""
    main_group_id = get_main_group_id()
    
    user = None
    
    # Handle ChatMember update (when bot is already in group)
    ...
    # Handle Message update with new_chat_members
...
async def filter_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Save user when they send message (to ensure they are in DB)
...
async def escrow_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check if second user exists in database
    u2_data = get_user_by_username(u2_username_raw)
    
    # Also check if user is admin
    admin_clean = ADMIN_USERNAME.replace('@', '').lower()
    if not u2_data and u2_username_raw.lower() == admin_clean:
        # Admin is not in database yet, but that's okay
        # We'll proceed but need to handle carefully
        u2_data = (ADMIN_ID, ADMIN_USERNAME) if 'ADMIN_ID' in dir() else None
    ...
    logger.info(f"Escrow created: Room {room['room_num']} - {u1_username} with {u2_username}")
...
async def role_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Role selection - give address or wait message"""
