...
async def get_invite_link(context: ContextTypes.DEFAULT_TYPE, room_id: int) -> str:
    ...
    try:
        ...
        invite_link = await context.bot.create_chat_invite_link(
            ...
            expire_date=int(time.time()) + 3600  # Expires in 1 hour
        )
        return invite_link.invite_link
    except Exception as e:
        logger.error(f"Failed to create invite link: {e}")
        
        # Try to export existing invite link as fallback
        try:
            invite_link = await context.bot.export_chat_invite_link(chat_id=room_id)
            return invite_link
        except Exception as e2:
            logger.error(f"Failed to export invite link: {e2}")
            return f"❌ Failed to generate link. Please add bot as admin with invite permission."
...
async def escrow_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(
            ...
            text=(
                ...
                f"🔗 Invite Link (Share with buyer):\n{invite_link}\n\n"
                f"📝 Commands:\n"
                f"/complete\n"
                f"/cancel"
            )
    ...
    seller_text = (
        ...
        f"👤 Buyer: @{buyer_username}\n"
        f"🔗 Share this link with buyer:\n{invite_link}\n\n"
        f"⚠️ Link expires in 1 hour or after 1 use!\n\n"
    ...
    await msg.reply_text(
        ...
        f"👤 Seller: @{seller_username}\n"
        f"👤 Buyer: @{buyer_username}\n\n"
        f"📩 Room invite link sent in DM to seller."
    )
