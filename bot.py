...
async def handle_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Save to database with verified = 0 (exact username, no lowercase)
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
...
async def username_change_check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = msg.from_user
    new_username = user.username if user.username else user.first_name
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if exists:
        ...
        c.execute("UPDATE users SET verified=1, username=? WHERE user_id=?", (username, user.id))
    else:
        # Insert new user as verified
...
async def escrow_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    c.execute("SELECT user_id, verified, username FROM users WHERE LOWER(username)=?", (u2_username_raw.lower(),))
    u2_data = c.fetchone()
    conn.close()

    if not u2_data:
        await msg.reply_text(f"❌ User @{u2_username_raw} not found! Ask them to /start first.")
        return
    
    u2_id = u2_data[0]
    u2_verified = u2_data[1]
    u2_username = u2_data[2]
    
    if u2_verified == 0:
        await msg.reply_text(f"❌ User @{u2_username_raw} verified nahi hai! Unhe pehle /start karna hoga.")
        return
